...
const main = async () => {
  ...
  const username = await askQuestion("Enter Username", "admin");
  const password = await askQuestion("Enter Password", "admin");

  const currentContext = execSync("docker context show").toString().trim();
  const changeContext = await askQuestion(
    `Do you want to change docker context? (Current: ${currentContext}) (y/n)`,
    "n",
  );

  if (changeContext.toLowerCase() === "y") {
    try {
      const contexts = execSync('docker context ls --format "{{.Name}}"')
        .toString()
        .trim()
        .split("\n");
      console.log("\nAvailable contexts:");
      ...
      if (selectedContext) {
        ...
        console.log(
          "Invalid context selection. Continuing with current context.",
        );
      }
    } catch (error) {
  ...
  const deployProcess = spawn(
    "docker",
    ["stack", "deploy", "-c", "docker-stack.local.yml", "infra"],
    {
      env: env,
      stdio: "inherit",
    },
  ...
  deployProcess.on("close", (code) => {
    ...
    if (code === 0) {
      ...
      console.log(`pgAdmin available at: http://<docker vm ip>:5001`);
    } else {
